    ...
    <section>
      <ul class="commom">
        <li
          ...
          :key="index"
          @click="check_det(item.applyId,item.state)"
        >
          <p>
            <span>{{item.name}}</span>
          ...
          <div>
            ...
            <button @click="continue_btn(item.applyId,item.type,item.state)" v-if="item.state==2">编辑</button>
            <p v-if="item.state==-1" @click="continue_btn(item.applyId,item.type,item.state)">
              <img
...
export default {
  ...
  methods: {
    ...
    // 点击编辑和申请奖励
    continue_btn(applyId, type, state) {
