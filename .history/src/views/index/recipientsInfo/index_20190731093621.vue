  ...
  <div class="page-index">
    ...
    <div class="recipient_form">
      ...
      <van-cell-group>
        ...
        />
        <van-field v-model="age" label="年龄:" placeholder="根据身份证识别年龄" type="number" disabled />
      </van-cell-group>
      ...
      <p>
        ...
        <span
          v-if="area === '请选择地区'"
          @click="selectAddress"
          :style="{area === '请选择地区'?'font-size:0.35rem;color:#ccc':'font-size:0.36rem;color:#999'}"
        >{{area}}</span>
        <span @click="selectAddress" style="font-size:0.36rem;color:#999;" v-else>{{area}}</span>
      </p>
      <div class="IdCard">
        <p>上传受助人身份证或户口本照片</p>
        <div>
  ...
  methods: {
    ...
    async testIdCard(data) {
      ...
      if (!realName(this.name)) {
        this.$dialog({ message: "请正确填写真实姓名" });
        return false;
      }

      if (this.grade === "") {
        this.$dialog({ message: "请选择班级" });
        return false;
      }
