          ...
          <ul>
            <li v-for="(item,index) in instituList" :key="index">
              <div>
                ...
                <router-link
                  :to="{ path: '/helpProgram/groupResets/addInstitutional',query:{organId:item.organId,auditStatus:auditStatus}}"
                  tag="p"
                >编辑</router-link>
              </div>
            </li>
          </ul>
          <div class="instroce" @click="addInstitutional">
            <van-icon name="plus" />
            <span>新增机构信息</span>
          </div>
        </div>
          ...
          <div class="second_from">
            <ul>
              <li v-for="(item,index) in recipientsDTOS" :key="index">
                <img :src="item.photo" alt @click="RecipientsList(index,item,item.recipientsId)" />
                <div @click="RecipientsList(index,item,item.recipientsId)">
                  <span>{{item.name}}</span>
...
export default {
  ...
  methods: {
    ...
    async myLaunchDetails(projectId) {
      ...
      if (res.code === 200) {
        this.instituList = res.data.organBaseInfoVOS;
        this.data = res.data;
        this.infomation = res.data.auditDesc;
        this.auditStatus = res.data.auditStatus;
        this.checkedArr = [...res.data.organIds];
    ...
    // 修改受助人信信息
    RecipientsList(index, item, recipientsId) {
    ...
    // 点击去添加机构信息
    addInstitutional() {
      this.$router.push({ path: "/helpProgram/groupResets/addInstitutional" });
    },
    ...
    async confirmData() {
      ...
      if (this.checkStatus == 2) {
        var res = await service.updataApply(this.data);
      } else {
        var res = await service.groupApply(this.data);
      }
      if (res.code === 200) {
        Storage.remove("dataHelp");
        this.active = 3;
