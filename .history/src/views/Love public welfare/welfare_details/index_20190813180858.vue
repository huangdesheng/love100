      ...
      <div class="banner">
        <van-swipe :autoplay="3000" indicator-color="white">
          <van-swipe-item v-for="(item,index) in welfarDetail.imageUrls" :key="index">
            <img :src="item" alt width="100%" @click="seeBigTu(item,welfarDetail.imageUrls)" />
          </van-swipe-item>
        </van-swipe>
        <button v-if="welfarDetail.status === 0">筹款中</button>
        <button v-if="welfarDetail.status === 1" style="background:#ccc">筹款结束</button>
        <div>
          <div>
      ...
      <div class="welfare_content">
        <div>
          <div>
            <h2>项目介绍</h2>
            <p>{{welfarDetail.introduce}}</p>
            <img :src="controlPic" alt width="100%" @click="singleBigTu(controlPic)" />
          </div>
          <div>
            <h2>项目预算</h2>
            <div>
              <h3>1、项目费用</h3>
              <p>{{welfarDetail.projectBudget}}</p>
              <img
                :src="welfarDetail.budgetPhoto"
                alt
                width="100%"
                @click="singleBigTu(welfarDetail.budgetPhoto)"
              />
            </div>
            <div>
              <h3>2、管理费说明</h3>
              <p>{{welfarDetail.explain}}</p>
            </div>
            <div>
              <h3>3、执行计划</h3>
              <p>{{welfarDetail.implementation}}</p>
            </div>
          </div>
        </div>
      </div>
