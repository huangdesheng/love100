    ...
    async moreUploadImg(e, index) {
      ...
      if (resolve.code === 200) {
        ...
        if (index === 3) {
          ...
          // this.item.photos[1].urls = this.familyPicMore;
          let familyItem = []
          this.item.photos[1].urls = this.familyPicMore.forEach(element => {
            familyItem.push(element.photoUrl)
          });
          console.log(familyPicMore)
        } else if (index === 4) {
          this.studyloadingStatus = false;
          this.studyPic = [...this.studyPic, ...resolve.data];
          let stydyPicMore = this.studyPic.slice(0, 3);
          this.stydyPicMore = stydyPicMore;
          let studyItem = []
          this.item.photos[2].urls = this.stydyPicMore.forEach(element => {
